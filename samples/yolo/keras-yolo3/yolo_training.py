...
def _main():
    ...
    isaac_graph_filename = os.path.join(base_dir, "yolo_training_apps.json")
    isaac_config_filename = os.path.join(base_dir, "yolo_training_apps.config.json")
    app = Application(
        name="yolo_training", modules=[
            "ml",
            "perception",
            "sight",
            "viewers",
        ])
    app.load_graph(isaac_graph_filename)
    app.load_config(isaac_config_filename)

    node = app.find_node_by_name("object_detection_training_samples")
...
def data_generator(bridge, batch_size, input_shape, anchors, num_classes):
    ...
    while True:
        image_data = []
        box_data = []
        if bridge.get_sample_number() < batch_size:
            continue
