...
Trains a neural network which takes in MFCC, deltas of order 1 and order 2 as input and outputs
keyword-wise probabilities. The network architecture used is Nvidia's ListeNet.
...
Command line arguments:
  ...
  --dropout DROPOUT     Dropout value used for training listenet. Default: 0.3
...
kCheckpointFileName = 'isaac_vcd_listenet'
# Name of the generated metadata file
kMetadataFileName = 'isaac_vcd_listenet.metadata.json'
# Name format of the intermediate keras models
kCheckpointAllFileName = 'isaac_vcd_listenet_{epoch:02d}_{loss:4f}_{val_loss:.4f}_{val_acc:.4f}.h5'
# Op names
kOutputOpName = 'output_node'


def glu(input):
    """Creates a convolutional gated linear unit for the input."""
    conv1 = Conv2D(64, (3, 3), padding='same', activation='linear', use_bias=False)(input)
    bn1 = BatchNormalization(axis=-1)(conv1)
    linear = Activation('linear')(bn1)

    conv2 = Conv2D(64, (3, 3), padding='same', activation='linear', use_bias=False)(input)
    bn2 = BatchNormalization(axis=-1)(conv2)
    gate = Activation('sigmoid')(bn2)

    gated_linear = Multiply()([linear, gate])
    return gated_linear


def listenet(num_classes):
    """ListeNet audio classification network architecture.
    ...
    glu1 = glu(input_tensor)
    glu1 = glu(glu1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(glu1)
    #    drop1 = Dropout(0.3)(pool1)

    glu2 = glu(pool1)
    glu2 = glu(glu2)
    pool2 = MaxPooling2D(pool_size=(2, 2))(glu2)
    drop2 = Dropout(args.dropout)(pool2)

    conv3 = Conv2D(256, (3, 3), padding='same', activation='linear', use_bias=True)(drop2)
    pool3 = MaxPooling2D(pool_size=(1, 3))(conv3)
    flat3 = Reshape((-1, 256))(pool3)

    gru4 = Bidirectional(GRU(64, activation='sigmoid'))(flat3)
    drop4 = Dropout(args.dropout)(gru4)

    output_tensor = Dense(num_classes, activation='sigmoid', name='output_layer')(drop4)
...
def load_local_dataset(path):
    ...
    Returns:
        numpy arrays for input audio, labels and the dataset scalers (mean and variance).
    """
    with h5py.File(path, 'r') as hf:
        input_audio = np.array(hf.get('input'))
        labels = np.array(hf.get('labels'))
        scaler = np.array(hf.get('scaler'))

    return input_audio, labels, scaler


def train_listenet(model, input_audio, labels, input_audio_validation, labels_validation):
    """Train the listenet network using input_audio and labels as training dataset."""
...
def continue_train_listenet(model, input_audio, labels, input_audio_validation, labels_validation):
    """Train the listenet network from a checkpoint using input_audio and labels as
    training dataset.
    """
    # Generate class weights
    class_weights = labels.sum(axis=0).max() / labels.sum(axis=0)
...
def generate_metadata(tf_config, scaler, input_audio, labels):
    """Generates metadata file containing scalers, sample rate and thresholds of each keyword"""
    K.clear_session()
    ...
    input_tensor_shape = [1, DataConfig.kWindowLength, DataConfig.kNumMfcc, DataConfig.kNumFeatures]
        ...
        [('<feature_extraction_node_name>', OrderedDict(
            [('isaac.audio.VoiceCommandFeatureExtraction', OrderedDict(
                [('sample_rate', DataConfig.kSampleRate),
                 ...
                 ('num_mfcc', DataConfig.kNumMfcc),
                 ('start_coefficient', DataConfig.kStartCoefficient),
                 ('hop_size', DataConfig.kHopLength),
                 ('window_length', DataConfig.kWindowLength),
                 ('num_classes', num_classes),
                 ('classes', args.keywords_list),
                 ('mean', scaler[0].reshape((-1)).astype(np.float32).tolist()),
                 ('sigma', scaler[1].reshape((-1)).astype(np.float32).tolist())]))
            ])),
...
def parse_command_line_arguments():
    ...
    parser.add_argument(
        '--dropout', dest='dropout', help='Dropout value used for training listenet')
    parser.add_argument(
        '--checkpoint',
        dest='checkpoint',
...
def main():
    ...
    if args.checkpoint is None:
        ...
        # Create the network.
        listenet_network = listenet(len(args.keywords_list))
    else:
        ...
        try:
            listenet_model = load_model(args.checkpoint)
        except ValueError as err:
            logging.error(('Input file specified ({}) only holds the weights, ' +
    ...
    input_audio, labels, scaler = load_local_dataset(join(args.tmpdir, 'features', 'train.hdf5'))
    input_audio = (input_audio - scaler[0]) / scaler[1]

    # Load validation dataset
    input_audio_validation, labels_validation, _ = load_local_dataset(
        join(args.tmpdir, 'features', 'valid.hdf5'))
    input_audio_validation = (input_audio_validation - scaler[0]) / scaler[1]

    if args.checkpoint:
        # Continue training from checkpoint
        continue_train_listenet(listenet_model, input_audio, labels, input_audio_validation,
                                labels_validation)
    else:
        # Train the model
        train_listenet(listenet_network, input_audio, labels, input_audio_validation,
                       labels_validation)

    # Generate the metadata
    generate_metadata(tf_config, scaler, input_audio_validation, labels_validation)

    # Convert the keras model to tensorflow
    convert_keras_to_tensorflow(tf_config)

    del input_audio, labels, scaler
