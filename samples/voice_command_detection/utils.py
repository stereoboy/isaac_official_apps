...
(MFCC, delta of order 1 and delta of order 2) and scalers (mean and standard deviation) from
the datasets which are used to train the network.
"""
...
def mfcc(y, sample_rate, n_ceps, n_mels, n_fft, hop_length, start):
    """Compute Mel-Frequency Cepstral Coefficients for a time-series data.
    Reference: https://en.wikipedia.org/wiki/Mel-frequency_cepstrum

    Args:
        y: Numpy array of audio samples.
        sample_rate: Sampling rate of audio.
        n_ceps: Number of MFCC coefficients.
        n_mels: Number of mels.
        n_fft: Length of FFT window.
        hop_length: Stride between consecutive FFT windows.
        start: Index of starting coefficient.
    Returns:
        2-d array of MFCC coefficients with shape (n_ceps, t).
    """
    ...
    mfcc = librosa.feature.mfcc(S=mel_spectrogram, n_mfcc=n_ceps + 1)[start:start + n_ceps]
    return mfcc
...
def augment_data(in_dir,
    ...
    kTimeStretchRate = [1.05, 1.10, 1.01, 1.07, 1.15]    #TODO: Decide with time_stretch_rate
...
def extract_features(data_dir, feature_path, keyword_id_map):
    """Extract MFCC, deltas of order 1 and order 2 features from the audio files. Also computes
    the mean and standard deviation scalers on the entire dataset
    """
    logging.debug('==================================================================')
    ...
    for filename in tqdm(audio_files, desc='Extracting', dynamic_ncols=True):
        ...
        if audio.shape[0] == 0:
            ...
            feature_mfcc = mfcc(audio, DataConfig.kSampleRate, DataConfig.kNumMfcc,
                                DataConfig.kNumMels, DataConfig.kFftLength,
                                DataConfig.kHopLength, 1)[:, :DataConfig.kWindowLength]

            feature_delta1 = librosa.feature.delta(feature_mfcc, order=1)
            feature_delta2 = librosa.feature.delta(feature_mfcc, order=2)
            features = np.stack((feature_mfcc.T, feature_delta1.T, feature_delta2.T), axis=-1)

            y = np.zeros(num_classes)
            if len(features) != DataConfig.kWindowLength:
                failed_count = failed_count + 1
                logging.error('Failed to extract features for {}'.format(filname))
            else:
                y[classIdx] = 1
    ...
    features_mean = np.mean(features_all, axis=0)
    features_std = np.std(features_all, axis=0)
    scaler = np.stack((features_mean, features_std))
