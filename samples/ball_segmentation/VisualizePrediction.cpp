...
void VisualizePrediction::tick() {
  ...
  bool ok = FromProto(image_tensor_reader, image_tensor_);
  if (!ok) {
    return;
  }
  ok = FromProto(segmentation_prediction_reader.getPrediction(), prediction_);
  if (!ok) {
    return;
  }

  const size_t tensor_rows = prediction_.size<0>();
  const size_t tensor_cols = prediction_.size<1>();
  const size_t image_tensor_rows = image_tensor_.size<0>();
  const size_t image_tensor_cols = image_tensor_.size<1>();
  ...
  for (size_t row = 0; row < tensor_rows; row++) {
    for (size_t col = 0; col < tensor_cols; col++) {
      probability_image_(row, col) = prediction_(row, col, 0);
    }
  }

  // generate segmentation image and join them side by side
  NormalizedTensorToImage(image_tensor_, ImageToTensorNormalization::kPositiveNegative,
                          left_image_);
