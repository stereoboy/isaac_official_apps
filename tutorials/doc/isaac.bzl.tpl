...
load("@isaac//engine/build/style:cpplint.bzl", "cpplint")
load("@isaac//engine/build/sphinx:sphinx.bzl", "sphinx", "sphinx_dep")
...
def isaac_cc_module(
    ...
    default_deps = ["@isaac//engine/alice", "@isaac//engine/core", "//messages"]
    ...
    native.cc_binary(
        ...
        deps = ["@isaac//engine/alice/tools:module"] + deps + ["_" + name],
        tags = tags,
        linkshared = True,
        **kwargs
    )
...
def isaac_component(name, filename = None, deps = [], visibility = [], **kwargs):
    ...
    isaac_cc_library(
        ...
        hdrs = [filename + ".hpp"],
        visibility = visibility,
        deps = deps + ["@isaac//engine/alice", "@isaac//engine/core", "//messages"],
        **kwargs
    )
...
_isaac_app_runscript = rule(
    ...
    attrs = {
        ...
        "template": attr.label(
            default = Label("@isaac//engine/alice/tools:run.sh.tpl"),
            allow_single_file = True,
        ),
    },
...
def isaac_app(name, modules = [], app_json_file = None, data = [], script = None, tags = [], excludes = [], remap_paths = {}, **kwargs):
    ...
    native.sh_binary(
        ...
        name = name,
        srcs = [script],
        data = ["@isaac//engine/alice/tools:main"] + [app_json_file] + module_sos + data,
        **kwargs
    )
    ...
    native.cc_test(
        ...
        deps = ["@isaac//engine/alice/tools:check_json"],
...
def isaac_pkg(name, srcs, remap_paths = {}, excludes = [], **kwargs):
    ...
    _isaac_pkg(
        name = name,
        srcs = srcs + ["@isaac//engine/build:run_script"],
