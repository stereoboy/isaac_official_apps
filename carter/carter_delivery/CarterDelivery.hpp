...
class CarterDelivery : public alice::Codelet {
 public:
  ...
  // Variable that indicates whether Carter arrived at the target
  ISAAC_PARAM(std::string, goto_has_arrived, "go_to/isaac.navigation.GoTo/has_arrived");
  // Variable that indicates whether Carter is stationary
  ISAAC_PARAM(std::string, goto_is_stationary, "go_to/isaac.navigation.GoTo/is_stationary");
  // Variable containing the distance from Carter to the target
  ISAAC_PARAM(std::string, goto_remaining_distance, "go_to/isaac.navigation.GoTo/remaining_delta_pos");  // NOLINT
  ...
  // The map waypoint which is used for picking up soda
  ISAAC_PARAM(std::string, soda_pickup_waypoint, "soda_pickup");
