...
// TODO Enable this once we have access to the map
...
// Name of a stopwatch to make sure we don't start checking if we arrived too early.
...
// Time interval we are waiting before we check if we arrived
...
constexpr char kStateAproachArm[] = "kAproachArm";
constexpr char kStateArmServerHuman[] = "kArmServerHuman";
constexpr char kStateArmServerJaco[] = "kArmServerJaco";
...
bool ExtractOrder(const std::string& text, std::string& waypoint, std::string& item) {
  ...
  constexpr char kSodaOrder[] = "bring me soda to ";
  if (StartsWith(text, kPopcornOrder)) {
    waypoint = ToLowerCase(text.substr(sizeof(kPopcornOrder) - 1));
    item = "popcorn";
    return true;
  } else if (StartsWith(text, kSodaOrder)) {
    waypoint = ToLowerCase(text.substr(sizeof(kSodaOrder) - 1));
    item = "soda";
    return true;
  } else {
    waypoint = "";
...
void CarterDelivery::start() {
  ...
  stopwatch(kSleepHelper).setClock(node()->clock());
...
void CarterDelivery::createStateMachine() {
  ...
  machine_.addTransition(kStateIdle, kStateDriveToPickup,
      [this] {
        ...
        // TODO Enable this check once we have access to the map
        ...
        } else if (desired_item_ == "soda") {
          pickup_waypoint_ = get_soda_pickup_waypoint();
        } else {
  ...
  machine_.addTransition(kStateDriveToPickup, kStateWaitForPickup,
      [this] {
        // Wait at least a bit before checking if we arrived to make sure the desired waypoint got
        // propagated. // TODO This needs to be done better.
        if (stopwatch(kSleepHelper).read() < kWaitBeforeCheckForArrival) {
          return false;
        }
        // We check a variable if we have arrived
        const auto maybe_has_arrived = getVariable(get_goto_has_arrived());
        if (!maybe_has_arrived) {
          LOG_WARNING("Could not read variable 'goto_has_arrived'");
          return false;
        }
        return (maybe_has_arrived > 0.5);
      },
      [] { });


  machine_.addState(kStateAproachArm,
      [this] {
        sendMessageToServer("Approaching pickup location. Please stand back.");
        stopwatch(kSleepHelper).start();
        auto* behavior = node()->app()->findComponentByName<navigation::SelectorBehavior>(
            "goal_behavior/isaac.navigation.SelectorBehavior");
        behavior->set_desired_behavior("fiducial");
      },
      [] { },
      [this] {
        auto* behavior = node()->app()->findComponentByName<navigation::SelectorBehavior>(
            "goal_behavior/isaac.navigation.SelectorBehavior");
        behavior->set_desired_behavior("waypoint");
      });

  machine_.addTransition(kStateWaitForPickup, kStateAproachArm,
      [this] {
        return desired_item_ == "soda";
      },
      [] { });

  machine_.addState(kStateArmServerHuman,
      [this] {
        sendMessageToCustomer("Waiting for server to place a soda on the robot");
        sendMessageToServer("Please put a soda on Carter");
      },
      [] { },
      [] { });

  machine_.addTransition(kStateAproachArm, kStateArmServerHuman,
      [this] {
        // Wait at least a bit before checking if we arrived to make sure the desired waypoint got
        // propagated. // TODO This needs to be done better.
        if (stopwatch(kSleepHelper).read() < kWaitBeforeCheckForArrival) {
  ...
  machine_.addState(kStateArmServerJaco,
      [this] {
        sendMessageToCustomer("Jaco is preparing your order.");
        outbox_.push(ChatMessage{"Carter", "kinova_jaco", "START"});
      },
      [] { },
      [this] {
        sendMessageToCustomer("Jaco has finished your order.");
      });

  machine_.addTransition(kStateArmServerHuman, kStateArmServerJaco,
      [this] {
        // The operator sends a message to indicate that he is ready
        if (!last_user_message_) {
          return false;
        }
  ...
  machine_.addTransition(kStateArmServerJaco, kStateDriveToDropoff,
      [this] {
        // The robot sends a message that he is ready
        if (!last_user_message_) {
          return false;
        }
        if (last_user_message_->user == "Jaco" && last_user_message_->channel == "kinova_jaco"
  ...
  machine_.addState(
      ...
      [this] {
        if (stopwatch(kDistanceUpdateHelper).interval(kDistanceUpdateInterval)) {
          const auto remaining = getVariable(get_goto_remaining_distance());
          if (!remaining) {
            LOG_ERROR("Could not read distance to target");
            return;
          }
          const int distance_int = 5 * static_cast<int>(0.2 * *remaining);
          if (distance_int <= 10) {
